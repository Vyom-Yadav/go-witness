...
//go:generate sh -c "go tool bpf2go -cc clang -target bpfel,bpfeb -go-package bpf -type orig_dst_key -type orig_dst_val -type orig_dst_key_v6 -type orig_dst_val_v6 -type witness_pid_ns_tid_key -type witness_pid_ns_tid_val -type comm_allowlist_key -type cgroup_allowlist_key -type tuple_key -type tuple_val -type tuple_key_v6 connect connect.bpf.c -- -I./headers -Wall -Werror $BPF_CFLAGS"
...
//go:generate sh -c "go tool bpf2go -cc clang -target bpfel,bpfeb -go-package bpf -type witness_pid_ns_tid_key -type witness_pid_ns_tid_val -type gate_key -type gate_val -type proxy_state_key -type control_val task_tracker task_tracker.bpf.c -- -I./headers -Wall -Werror $BPF_CFLAGS"
